...
function imp(buildImp, bidRequest, context) {
  ...
  delete imp.ext.tid // field moved to source
...
function request(buildRequest, imps, bidderRequest, context) {
  ...
  if (typeof request.app !== 'undefined') {
    ...
    if (typeof context.url !== 'undefined') {
      deepSetValue(request, 'site.page', context.url);
    }
    if (typeof context.publisherId !== 'undefined') {
  ...
  if (bidderRequest.eids) {
    deepSetValue(request, 'user.ext.eids', [...bidderRequest.eids]);
  }

  if (typeof deepAccess(request, 'device.sua') !== 'undefined') {
    deepSetValue(request, 'device.ext.sua', request.device.sua);
    delete request.device.sua; // moved 2.6 field to ext
  }
  if (typeof deepAccess(request, 'regs.gpp') !== 'undefined') {
    deepSetValue(request, 'regs.ext.gpp', request.regs.gpp);
    delete request.regs.gpp // moved 2.6 field to ext
  }
  if (typeof deepAccess(request, 'regs.gpp_sid') !== 'undefined') {
    deepSetValue(request, 'regs.ext.gpp_sid', request.regs.gpp_sid);
    delete request.regs.gpp_sid // moved 2.6 field to ext
  }

  if (bidderRequest) {
    if (typeof bidderRequest.coppa !== 'undefined') {
      deepSetValue(request, 'regs.coppa', bidderRequest.coppa === true ? 1 : (bidderRequest.coppa === false ? 0 : undefined));
    }
    ...
    if (bidderRequest.gdprConsent) {
      ...
      if (typeof bidderRequest.gdprConsent.gdprApplies !== 'undefined') {
        deepSetValue(request, 'regs.ext.gdpr', !!(bidderRequest.gdprConsent.gdprApplies));
      }
    ...
    if (typeof bidderRequest.tid !== 'undefined') {
      deepSetValue(request, 'source.tid', bidderRequest.tid);
    }

    if (typeof bidderRequest.schain !== 'undefined') {
      deepSetValue(request, 'source.ext.schain', bidderRequest.schain)
    }
  }
...
function bidResponse(buildBidResponse, bid, context) {
  ...
  if (typeof deepAccess(bid, 'ext.dsa') !== 'undefined') {
    deepSetValue(bidResponse, 'meta.dsa', bid.ext.dsa);
  }
...
export const spec = {
  ...
  buildRequests: (bidRequests, bidderRequest) => {
    Object.assign(bidderRequest, {
      schain: bidRequests.find(bidRequest => bidRequest.schain)?.schain,
      eids: bidRequests.find(bidRequest => bidRequest.userIdAsEids)?.userIdAsEids,
      tid: bidRequests.find(bidRequest => bidRequest.source?.tid)?.source.tid || bidRequests.find(bidRequest => bidRequest.ortb2Imp?.ext?.tid)?.ortb2Imp.ext.tid,
